...
///
/// Operand layout: `[mask, value, lane_or_delta, clamp]`. Inline-asm operand order is
/// `$0`=result, `$1`=value (i64, `l`), `$2`=lane/delta (i32, `r`),
/// `$3`=clamp (i32, `r`), `$4`=membermask (i32, `r`).
pub(crate) fn convert_shuffle_i64(
    ...
    if operands.len() != 4 {
        return pliron::input_err_noloc!(
            "Warp shuffle i64 requires 4 operands [mask, value, lane_or_delta, clamp]"
        );
    }
    let (mask, val, lane_or_delta, clamp) = (operands[0], operands[1], operands[2], operands[3]);
    ...
    let asm_op = inline_asm_convergent(
        ...
        rewriter,
        i64_ty.into(),
        vec![val, lane_or_delta, clamp, mask],
        &asm_template,
        "=l,l,r,r,r",
    );
