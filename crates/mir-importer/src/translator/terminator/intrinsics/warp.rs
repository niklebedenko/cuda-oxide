...
mod tests {
    ...
    #[test]
    fn legacy_shuffle_clamp_matches_cuda_width_encoding() {
        assert_eq!(
            legacy_shuffle_clamp_bits(LegacyShuffleMode::Xor, 32),
            Some(0x1f)
        );
        assert_eq!(
            legacy_shuffle_clamp_bits(LegacyShuffleMode::Xor, 16),
            Some(0x101f)
        );
        assert_eq!(
        ...
        assert_eq!(legacy_shuffle_clamp_bits(LegacyShuffleMode::Idx, 0), None);
        assert_eq!(legacy_shuffle_clamp_bits(LegacyShuffleMode::Down, 12), None);
    }
}
